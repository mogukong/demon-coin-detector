...
def fetch_klines(symbol, interval="4h", limit=1500):
...
def calc_indicators(klines):
    ...
    for i in range(len(klines)):
        if i < 18: ind.append(None); continue
        o1 = oi[i] if i < len(oi) else 0
        o4 = sum(oi[max(0,i-1):i])
        streak = 0
        for j in range(i,max(0,i-2),-1):
            if j<len(oi) and oi[j]>0: streak+=1
            else: break
        vs = v[max(0,i-6):i]; vm = sorted(vs)[len(vs)//2] if vs else 1
        vr = v[i]/max(vm,1)
        vs5 = v[max(0,i-30):i]; vm5 = sorted(vs5)[len(vs5)//2] if vs5 else 1
        vr5 = v[i]/max(vm5,1)
        c1 = (c[i]-c[i-1])/max(c[i-1],0.001)*100 if i>=1 else 0
        c4 = (c[i]-c[i-1])/max(c[i-1],0.001)*100 if i>=1 else 0
...
def sim(symbol, klines, tp_config):
    ...
    cooldown_until = 0
    fixed_pct = tp_config.get("fixed_pct", 0.5)
    fixed_at = tp_config.get("fixed_at", 0.50)
        ...
        if pos:
            ...
            if pnl <= -stop_loss:
                ...
                trades.append({**pos,"exit_price":closes[i],"exit_bar":i,"exit_time":klines[i][0],
                    "reason":"stop_loss","pnl":pnl*100*remaining,"hours":(i-pos["entry_bar"])*4,"peak":peak*100})
                pos = None; cooldown_until = i + 1; continue
            if not first_half_closed and pnl >= fixed_at:
                ...
                trades.append({**pos,"exit_price":closes[i],"exit_bar":i,"exit_time":klines[i][0],
                    ...
                    "hours":(i-pos["entry_bar"])*4,"peak":peak*100})
                ...
                if peak >= trail_activate:
                    ...
                    if drawdown >= trail_draw:
                        ...
                        trades.append({**pos,"exit_price":closes[i],"exit_bar":i,"exit_time":klines[i][0],
                            ...
                            "pnl":second_pnl,"hours":(i-pos["entry_bar"])*4,"peak":peak*100})
                        pos = None; cooldown_until = i + 1; continue
            if i-pos["entry_bar"] >= timeout_bars:
                remaining = 1.0 if not first_half_closed else (1.0 - fixed_pct)
                trades.append({**pos,"exit_price":closes[i],"exit_bar":i,"exit_time":klines[i][0],
                    "reason":"timeout","pnl":pnl*100*remaining,"hours":(i-pos["entry_bar"])*4})
                pos = None; cooldown_until = i + 1
    return trades
...
TP_CONFIGS = {
    "fixed50":          {"name":"固定50%全仓",   "fixed_pct":0,   "fixed_at":0.50, "trail_activate":0.10, "trail_draw":0.15},
    "trail_10_15":      {"name":"追踪10/15%全仓","fixed_pct":0,   "fixed_at":0.50, "trail_activate":0.10, "trail_draw":0.15},
    "S50_30_T10_15":    {"name":"50%@+30%+追踪10/15", "fixed_pct":0.5, "fixed_at":0.30, "trail_activate":0.10, "trail_draw":0.15},
    "S50_50_T10_15":    {"name":"50%@+50%+追踪10/15", "fixed_pct":0.5, "fixed_at":0.50, "trail_activate":0.10, "trail_draw":0.15},
    "S30_30_T10_15":    {"name":"30%@+30%+追踪10/15", "fixed_pct":0.3, "fixed_at":0.30, "trail_activate":0.10, "trail_draw":0.15},
    "S30_20_T10_15":    {"name":"30%@+20%+追踪10/15", "fixed_pct":0.3, "fixed_at":0.20, "trail_activate":0.10, "trail_draw":0.15},
}

def run_backtest(days, interval, limit):
    print(f"\n{'='*90}")
    print(f"  {days}天回测 ({interval} K线, {limit} bars)")
    print("="*90)
    ...
    for coin in COINS:
        ...
        kl = fetch_klines(coin, interval, limit)
        if not kl or len(kl) < 50: print("SKIP"); continue
        actual_days = len(kl) * (4 if interval == "4h" else 1) // 24
        print(f"OK({actual_days}d)")
        coin_count += 1
    ...
    print(f"\n  统计 ({coin_count} coins)")
    print(f"  {'策略':<28} {'交易':<6} {'胜率':<7} {'均PnL':<8} {'累计%':<9} {'最大盈':<8} {'最大亏':<9} {'止损':<5}")
    print("  " + "-"*85)
    
    best_name = ""; best_tot = -9999
    
    for cfg_name, cfg in TP_CONFIGS.items():
        st = all_stats[cfg_name]
        marker = ""
        if cfg_name == "S50_50_T10_15": marker = " ⭐"
        if st["tot"] > best_tot:
            best_tot = st["tot"]; best_name = cfg["name"]
        print(f"  {cfg['name']:<28} {st['n']:<6} {st['wr']:<7.1f} {st['avg']:<8.1f} {st['tot']:<9.1f} {st['mxw']:<8.1f} {st['mxl']:<9.1f} {st['sl']:<5}{marker}")
    
    print(f"\n  🏆 最优: {best_name} ({best_tot:.1f}%)")
    s50 = all_stats["S50_50_T10_15"]
    print(f"  ⭐ 50%@+50%+追踪: {s50['tot']:.1f}% | 胜率{s50['wr']:.1f}% | 止损{s50['sl']}次")
    
    return all_stats

# 90天用4h K线
run_backtest(90, "4h", 1500)
