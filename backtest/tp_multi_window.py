...
def fetch_klines(symbol, interval="1h", limit=2200):
        ...
        if pos:
            ...
            if pnl <= -stop_loss:
                ...
                trades.append({**pos,"exit_price":closes[i],"exit_bar":i,"exit_time":klines[i][0],
                    "reason":"stop_loss","pnl":pnl*100*remaining,"hours":i-pos["entry_bar"],"peak":peak*100})
            ...
            if not first_half_closed and pnl >= fixed_at:
                ...
                trades.append({**pos,"exit_price":closes[i],"exit_bar":i,"exit_time":klines[i][0],
                    ...
                    "hours":i-pos["entry_bar"],"peak":peak*100})
                ...
                if peak >= trail_activate:
                    ...
                    if drawdown >= trail_draw:
                        ...
                        trades.append({**pos,"exit_price":closes[i],"exit_bar":i,"exit_time":klines[i][0],
                            ...
                            "pnl":second_pnl,"hours":i-pos["entry_bar"],"peak":peak*100})
                        pos = None; cooldown_until = i + 2; continue
...
def run_backtest(days, limit):
    print(f"\n{'='*90}")
    print(f"  {days}天回测 ({limit} bars)")
    print("="*90)
    ...
    for coin in COINS:
        ...
        kl = fetch_klines(coin, "1h", limit)
        if not kl or len(kl) < 100: print("SKIP"); continue
        actual_days = len(kl) // 24
...
# 跑3个时间窗口
for days, limit in [(30, 750), (60, 1500), (90, 2200)]:
    run_backtest(days, limit)
