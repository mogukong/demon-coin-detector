...
def sim(symbol, klines, params):
    ...
    tp1_pct = params.get("tp1_pct", 0.50)  # 第一批止盈百分比
    tp1_sell = params.get("tp1_sell", 0.50)  # 第一批卖出比例
    trail_activate = params.get("trail_activate", 0.10)  # 追踪止盈启动
    trail_drawdown = params.get("trail_drawdown", 0.15)  # 追踪止盈回撤
    max_hold = params.get("max_hold", 168)  # 最大持仓时间
    position_mult = params.get("position_mult", 1.0)  # 仓位倍数
...
def stats(trades):
    if not trades: return {"n":0,"wr":0,"avg":0,"tot":0,"sl":0,"tp1":0,"tp2":0,"score_drop":0}
    wins=[t for t in trades if t["pnl"]>0]
    sl=sum(1 for t in trades if t.get("reason") in ["sl","sl_trail"])
    tp1=sum(1 for t in trades if t.get("reason")=="tp1")
    tp2=sum(1 for t in trades if t.get("reason")=="tp2")
    ...
    return {"n":len(trades),"wr":len(wins)/len(trades)*100,
        ...
        "tot":sum(t["pnl"] for t in trades),"sl":sl,"tp1":tp1,"tp2":tp2,"score_drop":score_drop}
...
# 基准配置 (组合B)
base = {"entry_score": 70, "sl_lock_trigger": 0.15, "sl_lock_target": 0.05, "score_exit": 55, "sq_bonus": 0}
...
configs = [
    ...
    # 止盈策略优化
    {"name": "TP1@40%", **base, "tp1_pct": 0.40},
    {"name": "TP1@60%", **base, "tp1_pct": 0.60},
    {"name": "TP1@70%", **base, "tp1_pct": 0.70},
    {"name": "TP1卖40%", **base, "tp1_sell": 0.40},
    {"name": "TP1卖60%", **base, "tp1_sell": 0.60},
    {"name": "TP1卖70%", **base, "tp1_sell": 0.70},
    
    # 追踪止盈优化
    {"name": "追踪8/12%", **base, "trail_activate": 0.08, "trail_drawdown": 0.12},
    {"name": "追踪10/15%", **base, "trail_activate": 0.10, "trail_drawdown": 0.15},
    {"name": "追踪12/18%", **base, "trail_activate": 0.12, "trail_drawdown": 0.18},
    {"name": "追踪15/20%", **base, "trail_activate": 0.15, "trail_drawdown": 0.20},
    {"name": "追踪20/25%", **base, "trail_activate": 0.20, "trail_drawdown": 0.25},
    
    # 利润锁定优化
    {"name": "锁10%→3%", **base, "sl_lock_trigger": 0.10, "sl_lock_target": 0.03},
    {"name": "锁10%→5%", **base, "sl_lock_trigger": 0.10, "sl_lock_target": 0.05},
    {"name": "锁12%→5%", **base, "sl_lock_trigger": 0.12, "sl_lock_target": 0.05},
    {"name": "锁15%→8%", **base, "sl_lock_trigger": 0.15, "sl_lock_target": 0.08},
    {"name": "锁20%→8%", **base, "sl_lock_trigger": 0.20, "sl_lock_target": 0.08},
    {"name": "锁20%→10%", **base, "sl_lock_trigger": 0.20, "sl_lock_target": 0.10},
    
    # 评分离场优化
    {"name": "评分<50离场", **base, "score_exit": 50},
    {"name": "评分<52离场", **base, "score_exit": 52},
    {"name": "评分<58离场", **base, "score_exit": 58},
    {"name": "评分<60离场", **base, "score_exit": 60},
    
    # 超时优化
    {"name": "超时72h", **base, "max_hold": 72},
    {"name": "超时120h", **base, "max_hold": 120},
    {"name": "超时240h", **base, "max_hold": 240},
    {"name": "超时336h", **base, "max_hold": 336},
    
    # 逼空信号
    {"name": "逼空+10", **base, "sq_bonus": 10},
    {"name": "逼空+15", **base, "sq_bonus": 15},
    {"name": "逼空+20", **base, "sq_bonus": 20},
    
    # 组合优化
    {"name": "最优A: TP1@60%+追踪15/20%+锁20%→8%", **base, "tp1_pct": 0.60, "trail_activate": 0.15, "trail_drawdown": 0.20, "sl_lock_trigger": 0.20, "sl_lock_target": 0.08},
    {"name": "最优B: TP1@60%+追踪15/20%+锁20%→10%", **base, "tp1_pct": 0.60, "trail_activate": 0.15, "trail_drawdown": 0.20, "sl_lock_trigger": 0.20, "sl_lock_target": 0.10},
    {"name": "最优C: TP1@70%+追踪20/25%+锁20%→10%", **base, "tp1_pct": 0.70, "trail_activate": 0.20, "trail_drawdown": 0.25, "sl_lock_trigger": 0.20, "sl_lock_target": 0.10},
    {"name": "最优D: TP1@60%+追踪15/20%+锁15%→8%+逼空15", **base, "tp1_pct": 0.60, "trail_activate": 0.15, "trail_drawdown": 0.20, "sl_lock_trigger": 0.15, "sl_lock_target": 0.08, "sq_bonus": 15},
    {"name": "最优E: TP1@60%+追踪20/25%+锁20%→10%+超时240h", **base, "tp1_pct": 0.60, "trail_activate": 0.20, "trail_drawdown": 0.25, "sl_lock_trigger": 0.20, "sl_lock_target": 0.10, "max_hold": 240},
]

print()
print(f"{'配置':<50} {'交易':<6} {'胜率':<7} {'均PnL':<8} {'累计%':<9} {'止损':<5} {'TP1':<5} {'TP2':<5}")
print("-"*100)
...
for cfg in configs:
    ...
    print(f"{cfg['name']:<50} {s['n']:<6} {s['wr']:<7.1f} {s['avg']:<8.1f} {s['tot']:<9.1f} {s['sl']:<5} {s['tp1']:<5} {s['tp2']:<5}")

# 找最优
print()
print("="*100)
print("TOP 10 配置 (按累计收益):")
results.sort(key=lambda x: x[1]["tot"], reverse=True)
for i, (name, s, cfg) in enumerate(results[:10]):
    print(f"  {i+1}. {name}: {s['tot']:.1f}% | 胜率{s['wr']:.1f}% | {s['n']}笔 | 止损{s['sl']} | TP1:{s['tp1']} TP2:{s['tp2']}")
...
print()
print("TOP 5 配置 (按胜率, 正收益且>=100%):")
results_wr = [(name, s, cfg) for name, s, cfg in results if s['tot'] >= 100 and s['n'] >= 50]
results_wr.sort(key=lambda x: x[1]["wr"], reverse=True)
for i, (name, s, cfg) in enumerate(results_wr[:5]):
