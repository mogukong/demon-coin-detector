    ...
    for i in range(72, len(klines)):
        ...
        if pos is None and i>=cooldown_until:
            ...
            if score>=55:
                ...
                peak=0; first_half_closed=False; sl_moved=False
        ...
        if pos:
            ...
            if mode == "trail_15_5":
                # 盈利15%后，止损移到盈利5%
                if pnl >= 0.15 and not sl_moved:
                    stop_loss = -0.05  # 止损从-6%移到+5%
                    sl_moved = True
                ...
                # 盈利10%后，止损移到盈利3%
                if pnl >= 0.10 and not sl_moved:
                    stop_loss = -0.03
                    sl_moved = True
            
            elif mode == "trail_20_10":
                # 盈利20%后，止损移到盈利10%
                if pnl >= 0.20 and not sl_moved:
                    stop_loss = -0.10
                    sl_moved = True
...
def stats(trades):
    if not trades: return {"n":0,"wr":0,"avg":0,"tot":0,"sl":0,"sl_rate":0}
...
modes = [
    ...
    ("trail_15_8", "盈利15%→止损8%"),
    ("trail_10_3", "盈利10%→止损3%"),
    ("trail_20_10", "盈利20%→止损10%"),
]

print(f"{'模式':<25} {'交易':<6} {'胜率':<7} {'均PnL':<8} {'累计%':<9} {'止损':<5} {'止损率':<7}")
print("-"*70)
...
for mode, name in modes:
    ...
    print(f"{name:<25} {s['n']:<6} {s['wr']:<7.1f} {s['avg']:<8.1f} {s['tot']:<9.1f} {s['sl']:<5} {s['sl_rate']:<7.1f}")

print()
print("移动止损说明:")
print("  盈利15%→止损5%: 涨15%后,止损从-6%移到+5%,锁定5%利润")
print("  盈利15%→止损8%: 涨15%后,止损从-6%移到+8%,锁定8%利润")
print("  盈利10%→止损3%: 涨10%后,止损从-6%移到+3%,锁定3%利润")
print("  盈利20%→止损10%: 涨20%后,止损从-6%移到+10%,锁定10%利润")
