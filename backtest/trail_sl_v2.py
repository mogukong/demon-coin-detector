...
def sim(symbol, klines, mode="no_trail"):
    """模拟交易"""
    ...
    for i in range(72, len(klines)):
        ...
        if pos:
            ...
            # 移动止损逻辑
            if mode == "no_trail":
                current_sl = base_sl  # 固定6%
            
            elif mode == "trail_15_5":
                # 盈利15%后，止损移到盈利5%
                if peak >= 0.15:
                    current_sl = -0.05  # 止损在+5%位置
                    sl_moved = True
            
            elif mode == "trail_15_8":
                # 盈利15%后，止损移到盈利8%
                if peak >= 0.15:
                    current_sl = -0.08
                    sl_moved = True
            
            elif mode == "trail_10_3":
                # 盈利10%后，止损移到盈利3%
                if peak >= 0.10:
                    current_sl = -0.03
                    sl_moved = True
            
            elif mode == "trail_20_10":
                # 盈利20%后，止损移到盈利10%
                if peak >= 0.20:
                    current_sl = -0.10
                    sl_moved = True
            
            # 止损检查 (pnl是负值，current_sl也是负值)
            # 当pnl <= current_sl时触发止损
            if pnl <= -current_sl:
...
def stats(trades):
    if not trades: return {"n":0,"wr":0,"avg":0,"tot":0,"sl":0,"sl_trail":0}
    ...
    return {"n":len(trades),"wr":len(wins)/len(trades)*100,
        ...
        "tot":sum(t["pnl"] for t in trades),"sl":sl,"sl_trail":sl_trail}
...
modes = [
    ("no_trail", "无移动止损 (固定6%)"),
    ("trail_15_5", "盈利15%→止损+5%"),
    ("trail_15_8", "盈利15%→止损+8%"),
    ("trail_10_3", "盈利10%→止损+3%"),
    ("trail_20_10", "盈利20%→止损+10%"),
]

print(f"{'模式':<25} {'交易':<6} {'胜率':<7} {'均PnL':<8} {'累计%':<9} {'止损':<5} {'移动止损':<8}")
print("-"*75)
...
for mode, name in modes:
    ...
    s = stats(all_trades)
    print(f"{name:<25} {s['n']:<6} {s['wr']:<7.1f} {s['avg']:<8.1f} {s['tot']:<9.1f} {s['sl']:<5} {s['sl_trail']:<8}")

print()
print("移动止损说明:")
print("  盈利15%→止损+5%: 涨15%后,止损从-6%移到+5%,锁定5%利润")
print("  盈利15%→止损+8%: 涨15%后,止损从-6%移到+8%,锁定8%利润")
print("  盈利10%→止损+3%: 涨10%后,止损从-6%移到+3%,锁定3%利润")
print("  盈利20%→止损+10%: 涨20%后,止损从-6%移到+10%,锁定10%利润")
