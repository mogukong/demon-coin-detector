...
def detect_regime(klines_slice):
    ...
    if max_spike > 0.08:
        ...
        return "trend" if trend_strength > 0.6 else "range"
    else:
        return "range"
...
def get_regime_params(regime):
    """根据市场状态返回参数"""
    params = {
        "trend": {"sl_mult": 1.5, "trail_act": 0.12, "trail_draw": 0.18, "timeout": 240},
        "range": {"sl_mult": 0.8, "trail_act": 0.08, "trail_draw": 0.12, "timeout": 120},
...
def estimate_ev(win_rate, avg_win, avg_loss):
    """计算EV"""
    cost = 0.0004*2 + 0.001 + 0.0001  # 手续费+滑点+资金费
    return win_rate * avg_win + (1-win_rate) * avg_loss - cost

def sim_v27(symbol, klines):
    """当前v2.7策略"""
    ...
    for i in range(72, len(klines)):
        ...
        if pos is None and i>=cooldown_until:
            ...
            if st_dir<0: pass_filter=False
                ...
                if rsi>80: pass  # 小仓位10%
                elif rsi>70: score-=10
                if score<55: pass_filter=False
            
            if pass_filter:
                pos={"entry_price":closes[i],"entry_bar":i,"entry_time":klines[i][0],"peak":0}
    ...
    for i in range(72, len(klines)):
        ...
        if pos is None and i>=cooldown_until:
            ...
            if pass_filter and recent_total >= 5:
                ...
                avg_win = 15.0  # 估算
                avg_loss = -5.0  # 估算
                ev = estimate_ev(win_rate, avg_win/100, avg_loss/100)
                if ev < 0.02:  # EV < 2% 不开仓
                    pass_filter = False
            ...
            if pass_filter:
                ...
                stop_loss = base_sl * regime_params["sl_mult"]
                trail_activate = regime_params["trail_act"]
                trail_draw = regime_params["trail_draw"]
                timeout = regime_params["timeout"]
            else:
                stop_loss = base_sl
                trail_activate = 0.10
                trail_draw = 0.15
...
for coin in COINS:
    ...
    v27_trades.extend(sim_v27(coin, kl))
    v3_trades.extend(sim_v3(coin, kl))
    time.sleep(0.1)

s27 = stats(v27_trades)
s3 = stats(v3_trades)

print("="*70)
print("v2.7 vs v3.0 高级框架回测对比 (26币 30天)")
print("="*70)
print(f"{'策略':<20} {'交易':<6} {'胜率':<7} {'均PnL':<8} {'累计%':<9} {'止损':<5} {'最大亏':<8}")
print("-"*70)
print(f"{'v2.7 当前':<20} {s27['n']:<6} {s27['wr']:<7.1f} {s27['avg']:<8.1f} {s27['tot']:<9.1f} {s27['sl']:<5} {s27['mxl']:<8.1f}")
print(f"{'v3.0 高级框架':<20} {s3['n']:<6} {s3['wr']:<7.1f} {s3['avg']:<8.1f} {s3['tot']:<9.1f} {s3['sl']:<5} {s3['mxl']:<8.1f}")

diff = s3['tot'] - s27['tot']
print()
print(f"差异: {diff:+.1f}% ({'v3更优' if diff > 0 else 'v2.7更优'})")
print()
print("v3.0新增功能:")
print("  ✅ 行情状态分仓位 (趋势/震荡/插针)")
print("  ✅ EV预估 (历史胜率滚动计算)")
print("  ✅ 动态止损/止盈/超时")
