...
def score_signal_v4(d, st_dir):
    ...
    # === 第二层: 趋势与价格结构 ===
    # SuperTrend
    if st_dir>0: s+=15; hard_confirm+=1
    elif st_dir<0: s-=10
    
    # 均线: EMA7 > EMA25 = 短期趋势向上
    if d["ema7"] > d["ema25"]: s+=10; hard_confirm+=1
    else: s-=5
    
    # 价格突破布林带
    if d["close"] > d["bb_upper"]: s+=5; hard_confirm+=1  # 突破上轨
    elif d["close"] < d["bb_lower"]: s-=5  # 跌破下轨
    ...
    # OI 4h
    if d["oi_chg_4h"]>8: s+=15; soft_confirm+=1
    elif d["oi_chg_4h"]>4: s+=10
    elif d["oi_chg_4h"]>0: s+=5
    elif d["oi_chg_4h"]<0: s-=10
    
    # OI连续上涨
    if d["oi_up_streak"]>=3: s+=10; soft_confirm+=1
    elif d["oi_up_streak"]>=2: s+=5
    ...
    # 成交量
    if d["vol_ratio"]>2.0: s+=10; soft_confirm+=1
    elif d["vol_ratio"]>1.4: s+=5
    
    # 价格变化
    if d["chg_4h"]>2: s+=10
    elif d["chg_4h"]>0: s+=5
    elif d["chg_4h"]<0: s-=10
    
    return s, hard_confirm, soft_confirm
...
def sim_v3(symbol, klines):
    ...
    for i in range(72, len(klines)):
        ...
        if pos:
            ...
            peak=max(peak,pnl)
            if pnl<=-stop_loss:
                trades.append({**pos,"exit_price":closes[i],"exit_bar":i,"pnl":pnl*100,"reason":"sl","peak":peak*100})
            ...
            if i-pos["entry_bar"]>=168:
                trades.append({**pos,"exit_price":closes[i],"exit_bar":i,"pnl":pnl*100,"reason":"to"})
                pos=None; cooldown_until=i+2
    return trades

def sim_v4(symbol, klines):
    """v4.0 OI资金流+分档止盈+多证据共振"""
    if len(klines)<100: return []
    ind,closes,volumes,st = calc_indicators(klines)
    trades=[]; pos=None; peak=0; cooldown_until=0
    stop_loss=0.05
    # 分档止盈: +3.5%平25%, +7.5%平25%, +12%平50%
    tp1_pct=0.035; tp1_close=0.25
    tp2_pct=0.075; tp2_close=0.25
    tp3_pct=0.12; tp3_close=0.50
    ...
    for i in range(72, len(klines)):
        ...
        d=ind[i]
        if pos is None and i>=cooldown_until:
            score, hard, soft = score_signal_v4(d, st[i])
            rsi=d["rsi"]
            # 多证据共振: 至少2个硬确认+1个软确认
            if hard<2 or soft<1: continue
            if score>=55:
                pos={"entry_price":closes[i],"entry_bar":i,"peak":0}
...
for coin in COINS:
    ...
    v4_trades.extend(sim_v4(coin, kl))
    time.sleep(0.1)
...
print("="*70)
print("v3.0 vs v4.0 理念吸收回测 (26币 30天)")
print("="*70)
print(f"{'策略':<20} {'交易':<6} {'胜率':<7} {'均PnL':<8} {'累计%':<9} {'止损':<5} {'最大亏':<8} {'效率':<8}")
print("-"*70)
eff3 = s3['tot']/s3['sl'] if s3['sl']>0 else 0
eff4 = s4['tot']/s4['sl'] if s4['sl']>0 else 0
print(f"{'v3.0 当前':<20} {s3['n']:<6} {s3['wr']:<7.1f} {s3['avg']:<8.1f} {s3['tot']:<9.1f} {s3['sl']:<5} {s3['mxl']:<8.1f} {eff3:<8.1f}")
print(f"{'v4.0 理念吸收':<20} {s4['n']:<6} {s4['wr']:<7.1f} {s4['avg']:<8.1f} {s4['tot']:<9.1f} {s4['sl']:<5} {s4['mxl']:<8.1f} {eff4:<8.1f}")

diff = s4['tot'] - s3['tot']
print()
print(f"收益差异: {diff:+.1f}% ({'v4更优' if diff > 0 else 'v3更优'})")
print(f"胜率差异: {s4['wr']-s3['wr']:+.1f}%")
print(f"止损差异: {s4['sl']-s3['sl']:+d}次")
print(f"效率差异: {eff4-eff3:+.1f}")
print()
print("v4.0新增:")
print("  ✅ OI资金流判断 (价格+OI组合)")
print("  ✅ 多证据共振 (2硬+1软)")
print("  ✅ 分档止盈 (+3.5%/+7.5%/+12%)")
print("  ✅ 均线+布林带+RSI")
