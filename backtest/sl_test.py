...
def sim(symbol, klines, sl_pct=0.05):
    """模拟交易"""
...
print(f"{'止损比例':<10} {'交易':<6} {'胜率':<7} {'均PnL':<8} {'累计%':<9} {'止损':<5} {'止损率':<7} {'效率':<8}")
print("-"*65)
...
for sl in sl_pcts:
    ...
    for coin, kl in all_klines.items():
        all_trades.extend(sim(coin, kl, sl))
    s = stats(all_trades)
    eff = s['tot']/s['sl'] if s['sl']>0 else 0
    results.append((sl, s, eff))
    print(f"{sl*100:.0f}%{'':<7} {s['n']:<6} {s['wr']:<7.1f} {s['avg']:<8.1f} {s['tot']:<9.1f} {s['sl']:<5} {s['sl_rate']:<7.1f} {eff:<8.1f}")
...
print(f"🏆 最高收益: {best[0]*100:.0f}%止损 ({best[1]['tot']:.1f}%)")
print(f"⭐ 最高效率: {best_eff[0]*100:.0f}%止损 (效率{best_eff[2]:.1f})")
