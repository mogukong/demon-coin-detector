...
def sim(symbol, klines, strategy_name):
    ...
    for i in range(72, len(klines)):
        ...
        if pos is None and i>=cooldown_until:
            ...
            if strategy_name == "v2.5_current":
                # 当前策略: ST>=0 + score>=55 + RSI<75 + RSI>30
            ...
            elif strategy_name == "A_tight_score":
                # 方案A: 提高评分到65
                if st_dir<0: pass_filter=False
                elif rsi>75: pass_filter=False
                elif rsi<30: pass_filter=False
            ...
            elif strategy_name == "B_rsi65":
                # 方案B: RSI>65就不开
                if st_dir<0: pass_filter=False
                elif rsi>65: pass_filter=False
                elif rsi<30: pass_filter=False
                elif score<55: pass_filter=False
            
            elif strategy_name == "C_vol_filter":
                # 方案C: 成交量<1.0不开
                if st_dir<0: pass_filter=False
                elif rsi>75: pass_filter=False
                elif rsi<30: pass_filter=False
            ...
            elif strategy_name == "D_oi_1h_filter":
                # 方案D: OI 1h下跌不开
                if st_dir<0: pass_filter=False
                elif rsi>75: pass_filter=False
                elif rsi<30: pass_filter=False
            ...
            elif strategy_name == "E_chg4h_filter":
                # 方案E: 4h价格下跌不开
                if st_dir<0: pass_filter=False
                elif rsi>75: pass_filter=False
                elif rsi<30: pass_filter=False
            ...
            elif strategy_name == "F_combined":
                # 方案F: 组合优化 (RSI>65不开 + OI 1h<0不开 + vol<1.0不开)
                if st_dir<0: pass_filter=False
                elif rsi>65: pass_filter=False
                elif rsi<30: pass_filter=False
                elif d["oi_chg_1h"]<0: pass_filter=False
...
STRATEGIES = [
    "v2.5_current", "A_tight_score", "B_rsi65",
    "C_vol_filter", "D_oi_1h_filter", "E_chg4h_filter", "F_combined"
]
STRAT_NAMES = {
    "v2.5_current":"当前v2.5(55分/RSI75)",
    "A_tight_score":"A:提高到65分",
    "B_rsi65":"B:RSI>65不开",
    "C_vol_filter":"C:成交量<1.0不开",
    "D_oi_1h_filter":"D:OI 1h下跌不开",
    "E_chg4h_filter":"E:4h价格下跌不开",
    "F_combined":"F:组合(RSI65+OI1h+Vol)",
}
...
print(f"{'='*85}")
print(f"信号优化回测 - {coin_count}个币 30天")
print(f"{'='*85}")
print(f"{'策略':<30} {'交易':<6} {'胜率':<7} {'均PnL':<8} {'累计%':<9} {'止损':<5} {'最大亏':<8}")
print("-"*85)

for s in STRATEGIES:
    st=stats(all_trades[s])
    nm=STRAT_NAMES[s]
    print(f"{nm:<30} {st['n']:<6} {st['wr']:<7.1f} {st['avg']:<8.1f} {st['tot']:<9.1f} {st['sl']:<5} {st['mxl']:<8.1f}")
